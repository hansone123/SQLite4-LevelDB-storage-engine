...
int sqlite4VdbeDecoderGetColumn(
    ...
    printf("varintsize of sizeofheader: %d\n",n);
...
static int encodeOneKeyValue(
    ...
    printf("In encodeOneKeyValue:\n");
    int flags = pMem->flags;
    int i;
    int n;
    int iStart = p->nOut;
    if (flags & MEM_Null) {
        printf("flags & MEM_Null\n");
        if (enlargeEncoderAllocation(p, 1)) return SQLITE4_NOMEM;
        p->aOut[p->nOut++] = 0x05; /* NULL */
    } else
        if (flags & (MEM_Real | MEM_Int)) {
        printf("flags & (MEM_Real|MEM_Int)\n");

        if (enlargeEncoderAllocation(p, 16)) return SQLITE4_NOMEM;
        encodeNumericKey(p, pMem->u.num);
    } else if (flags & MEM_Str) {
        printf("flags & MEM_Str\n");
        int enc; /* Initial encoding o (string)key:    

(string)value:  *** �tabletest3test3 CREATE TABLE test3 (col1 text primary key, col2 int)
...
f pMem */
        ...
        printf("/* A BLOB value that is followed by other values */\n");
...
int sqlite4VdbeEncodeKey(
    ...
    *pnOut = 0;
    ...
    if (rc == SQLITE4_OK && nExtra) {
        rc = enlargeEncoderAllocation(&x, nExtra);
    }
    if (rc) {
